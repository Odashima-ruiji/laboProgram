            ...
            if(Node[i].n_X == Node[i].n_xD && Node[i].n_Y == Node[i].n_yD ){  
                ...
                if(Node[i].stack_num > 0 && Node[i].p_on == 0 && i < po){
                    ...
                    do{
                        ...
                        int  numdata = pop(i);
                        stack_sort();
                        if(search(i,numdata) == 0){ // 同じデータがなければ
                    
                            Node[i].p_on = 3;
                            Node[i].n_xD = (numdata % (Ax*Ax) ) / Ax;
                            Node[i].n_yD =  numdata % Ax;
                            count_p_on_3 += 1;
                            if((Twait*Ax*Ax - numdata >  reftime*Ax*Ax) || (Node[i].n_xD - Node[i].n_X > refdistance || Node[i].n_X - Node[i].n_xD > refdistance) || (Node[i].n_yD - Node[i].n_Y > refdistance || Node[i].n_Y - Node[i].n_yD > refdistance)){  //長い時間と距離経過しているとき参照しない
...
void initial_detect_trans(){
	...
	for(int i=0 ;i<N_ALL_NUM ;i++){
		...
		if((double)Node[i].n_insec_X==Node[i].n_X && (double)Node[i].n_insec_Y==Node[i].n_Y){
			...
			/*//上下1交差点分の情報を取得
			if(Trans[Node[i].n_insec_X][Node[i].n_insec_Y].wp_Exist == 1){
				data = Ax*Ax*Twait + Ax*Node[i].n_X + Node[i].n_Y;
				push(i,data);
				if(i == 0){
					stack_push[push_num] = data;
					push_num++;
				}
				get_info += 1; //情報を取得したカウント
				//printf("ノード番号%d番 %d秒  座標(%d,%d)にて乗客発見!!\n",i,Twait,Node[i].n_X,Node[i].n_Y);
			}
			if((Node[i].n_insec_X)+1 < Ax && Trans[(Node[i].n_insec_X)+1][Node[i].n_insec_Y].wp_Exist == 1 ){
				data = Ax*Ax*Twait + Ax*(Node[i].n_X+1.0) + Node[i].n_Y;
				push(i,data);
				get_info += 1; //情報を取得したカウント
				//printf("ノード番号%d番 %d秒  座標(%d,%d)にて乗客発見!!\n",i,Twait,Node[i].n_X,Node[i].n_Y);
			}
			if((Node[i].n_insec_X)-1 >= 0 && Trans[(Node[i].n_insec_X)-1][Node[i].n_insec_Y].wp_Exist == 1){
				data = Ax*Ax*Twait + Ax*(Node[i].n_X-1.0) + Node[i].n_Y;
				push(i,data);
				get_info += 1; //情報を取得したカウント
				//printf("ノード番号%d番 %d秒  座標(%d,%d)にて乗客発見!!\n",i,Twait,Node[i].n_X,Node[i].n_Y);
			}
			if((Node[i].n_insec_Y)+1 < Ay && Trans[Node[i].n_insec_X][(Node[i].n_insec_Y)+1].wp_Exist == 1){
				data = Ax*Ax*Twait + Ax*(Node[i].n_X) + Node[i].n_Y+1.0;
				push(i,data);
				get_info += 1; //情報を取得したカウント
				//printf("ノード番号%d番 %d秒  座標(%d,%d)にて乗客発見!!\n",i,Twait,Node[i].n_X,Node[i].n_Y);
			}
			if((Node[i].n_insec_Y)-1 >= 0 && Trans[Node[i].n_insec_X][(Node[i].n_insec_Y)-1].wp_Exist == 1){
				data = Ax*Ax*Twait + Ax*(Node[i].n_X) + Node[i].n_Y-1.0;
				push(i,data);
				get_info += 1; //情報を取得したカウント
				//printf("ノード番号%d番 %d秒  座標(%d,%d)にて乗客発見!!\n",i,Twait,Node[i].n_X,Node[i].n_Y);
			}*/
		}else{//ノードが交差点でなく, 道路上にいるとき
