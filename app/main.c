...
/*
double	r		= 10.0;		// 送信範囲 [m]
double	v		= 1.0;		// 端末の移動速度の絶対値 [m/s]
double	dt		= 1.0;	        // 移動時間ステップ [s]
*/

int queue_head[Ax][Ay] ;  //交差点のキューの初期値設定
int queue_num[Ax][Ay] ;   //交差点のキューのデータ数
int p_add_count;   //追加した乗客の数
int all10count[10000];         //10秒ごとのすべての到達人数
int all_count[10000];
double range[15];
int range_count[15];
int sum_all10count[500];
double ave_all10count[500];
int traffic_counter2[10000]; //10秒ごとの交通量
int transmit__[N_ALL_NUM][P_ALL_NUM]; //通信した利用客を記憶する配列
double speed_conter2[10000]; //特定区間の速度を保持する関数
double n = 0 ;//実行回数

//確認用20231013
int stack_push[5000]; //スタックにpushされたデータを確認
int stack_pop[5000]; //スタックからpopされたデータを確認
int push_num = 0;
int pop_num = 0;
...
int check_3;
int check_4;
int goalstep[P_ALL_NUM]; //全員の到着ステップ格納
int o = 0;
int Twait = 0; //待ち時間
int allstep; //ステップ数の総和

int count_ifgo =0;  // 情報に従うノードがデータをもとに目的地を設定した回数
int count_getoff=0; //二人目の乗客をおろした回数
int count_ride1 = 0; //乗客が一人目を乗せた回数
int count_ride2 = 0; //乗客が二人目を乗せた回数
int total_transmit =0; //すべての情報交換回数
int count_ride3 = 0; //情報フローティングで得た情報をもとに乗客を拾った回数
int ride_transmit = 0; //エピデミック通信で他の車から共有された待ち客情報によって乗客を拾った回数
int count_p_no = 0; //情報に従うノードが目的地に到着したがいなかった回数
int count_p_on_3 = 0; //p_on = 3になる回数
int ridecount = 0; //乗客が乗っていたステップ
int waitcount = 0; //乗客が待っていたステップ
int distancemass=0;
int get_info = 0; //情報を取得したカウント
int traffic_counter = 0; //交通量をカウント
double speed_counter = 0; //速度を保持
double v_f = 40; //自由走行速度 [km/h]
double d_j = 0.12; //道路の飽和密度
double free_variable; 
// double tra_count_x_plus[Ax][Ay]; //縦方向道路の交通量を保持する関数
// double tra_count_x_minus[Ax][Ay]; //縦方向道路の交通量を保持する関数
// double tra_count_y_plus[Ax][Ay]; //横方向道路の交通量を保持する関数
// double tra_count_y_minus[Ax][Ay]; //横方向道路の交通量を保持する関数
double sum =0;
//int d[4][2];
...
double area_disaster = (Ax - 2*Ax_d)/2; //避難区域半径
...
/* gnuplotで出力-------------------------------------------------------------------------------------------------- */
    ...
    for(int jc=0;jc<1;jc++){
        ...
        while(P_check() < P_goal){ //乗客が残っているかの判定 0なら乗客が残っている 1なら残っていない
